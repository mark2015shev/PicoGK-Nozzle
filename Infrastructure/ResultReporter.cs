...
internal static class ResultReporter
{
    public static void Report(PipelineRunResult result)
    {
        ...
        if (result.Autotune != null)
        {
            ...
            Library.Log("Design: PHYSICS-INFORMED pre-size (NozzleGeometrySynthesis) — diameters/lengths/expander/stator from source + reduced-order synthesis rules; template yaw/pitch/count/wall kept.");
        ...
        if (result.CriticalRatios != null)
        {
            ...
            Library.Log("--- Four critical ratios (design envelope — approximate lumped metrics, not CFD) ---");
        ...
        if (result.SiFlow?.SwirlSegmentPhysics != null)
        {
            foreach (string line in result.SiFlow.SwirlSegmentPhysics.FormatDebugBlock().Split('\n'))
            {
                if (line.Length > 0)
                    Library.Log(line);
            }
        ...
        Library.Log($"CoreGasDensity ρ_core [kg/m3]: {s.CoreGasDensityKgPerM3:F4} (approximate ideal-gas blend when T_exhaust set)");
        ...
        Library.Log("--- Inlet suction / low-pressure (reduced-order lumped model — not CFD; not free energy) ---");
        ...
        Library.Log("--- Swirl-pressure recovery (expander) — reduced-order wall ΔP model, not CFD; not centrifugal thrust ---");
        Library.Log($"SwirlPressureRisePa:          {s.SwirlPressureRisePa:F1} (order rho*v_theta^2/r wall rise, bounded)");
        Library.Log($"[diagnostic_force_only] ExpanderWallAxialForceN: {s.ExpanderWallAxialForceN:F2} (lumped wall ΔP·A — not in FinalThrustN)");
        ...
        Library.Log($"ExpansionEfficiency [-]:      {s.ExpansionEfficiency:F3} (geometry + expansion ratio, reduced-order)");
        Library.Log($"AxialRecoveryEfficiency [-]:  {s.AxialRecoveryEfficiency:F3} (stator vs swirl angle match, bounded approximation, capped)");
    ...
    private static void LogChamberPhysicsSection(SiFlowDiagnostics sf)
    {
        ...
        Library.Log("--- Swirling diffuser / expander (bounded Cp-style recovery model) ---");
        ...
        Library.Log("--- Ejector operating regime (lumped classification) ---");
    ...
    private static void LogSiCouplingAudit(SiFlowDiagnostics sf)
    {
        ...
        Library.Log($"Capture ΔP_augment [Pa]:       {c.CaptureStaticPressureDeficitAugmentationPa:F1}  (core suction → entrainment driver)  Δp_core useful [Pa]: {c.DeltaPCoreUsefulForEntrainmentPa:F1}");
    ...
    {
        Library.Log("--- Vortex structure and pressure field (reduced-order — not CFD) [legacy compact] ---");
    ...
    private static void LogDocumentedModelAssumptions()
    {
        Library.Log("--- Documented reduced-order model assumptions (full list) ---");
        ...
        Library.Log("- Legacy reference solver: jet-speed blend of V_core×(A_source/A_inj) and mdot/(ρ_core A_inj); live SI uses InjectorDischargeSolver + march.");
        Library.Log("- SI swirl segment: entrainment demand from capture pressure deficit (Bernoulli-style V_ent) × lumped mixing effectiveness, plus small shear term; choked/compressible caps per step.");
    ...
    private static void LogDerivedSwirlChamberSizing(SwirlChamberSizingModel.SizingDiagnostics d)
    {
        ...
        string modeLabel = d.Mode switch
        {
            ...
            SwirlChamberSizingModel.DiameterMode.SynthesisHeuristic => "synthesis-derived (jet×swirl×ER approximate bore)",
        ...
        {
            Library.Log($"(Synthesis sizing path) ER used [-]: {d.TargetEntrainmentRatio:F4}  D_chamber [mm]: {d.ChamberDiameterTargetMm:F2}  A_inj/A_bore [-]: {d.InjectorToBoreAreaRatio:F4}");
        }
        else
        {
